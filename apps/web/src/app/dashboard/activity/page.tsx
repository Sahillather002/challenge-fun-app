...
export default function ActivityPage() {
  ...
  return (
    <div className="space-y-12 pb-24 max-w-7xl mx-auto px-4 sm:px-0">
      {/* Header */}
      <header>
        <h1 className="text-4xl font-black tracking-tight uppercase mb-2 text-foreground">Biometric Telemetry</h1>
        <p className="text-muted-foreground font-bold">Real-time neural tracking of kinetic energy and metabolic output</p>
      </header>
      ...
      <motion.div
        ...
        className="glass p-12 rounded-[3.5rem] border border-border/50 bg-muted/40 relative overflow-hidden group shadow-2xl"
      >
        <div className={`absolute inset-0 bg-gradient-to-tr ${isTracking ? 'from-emerald-500/20' : 'from-slate-500/10'} via-transparent to-teal-500/10 pointer-events-none transition-colors duration-1000`} />

        <div className="relative z-10 space-y-12 text-center">
          <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full glass border border-border/50 mb-4 backdrop-blur-xl">
            <div className={`w-3 h-3 rounded-full ${isTracking ? 'bg-emerald-500 animate-pulse shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-muted-foreground/30'}`} />
            <span className={`text-[10px] font-black tracking-[0.3em] uppercase ${isTracking ? 'text-emerald-500' : 'text-muted-foreground'}`}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-12">
            {[
              { label: 'Kinetic Link (Steps)', val: steps.toLocaleString(), icon: <Footprints size={24} className="text-emerald-500" /> },
              { label: 'Spatial Vector (KM)', val: distance.toFixed(2), icon: <TrendingUp size={24} className="text-teal-500" /> },
              { label: 'Thermal Loss (Cal)', val: Math.floor(calories), icon: <Flame size={24} className="text-orange-500" /> },
              { label: 'Temporal sync (Min)', val: Math.floor(activeMinutes), icon: <Calendar size={24} className="text-emerald-600" /> },
            ].map((stat, i) => (
              <div key={i} className="flex flex-col items-center">
                <div className="p-4 glass rounded-2xl mb-4 text-emerald-500 shadow-inner group-hover:scale-110 transition-transform">
                  {stat.icon}
                </div>
          ...
          <div className="flex items-center gap-6 justify-center pt-8">
            <Button
              ...
              className={`h-16 px-12 rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] shadow-2xl transition-all hover:scale-105 active:scale-95 ${isTracking
                ? 'bg-red-500/10 text-red-500 border border-red-500/20 flex gap-4'
                : 'btn-gradient text-primary-foreground flex gap-4'
                }`}
            >
            ...
            <button
              onClick={handleReset}
              type="button"
              className="w-16 h-16 rounded-[2rem] glass flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors border border-border/50"
            >
              <RotateCcw size={20} />
            </button>
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        >
          ...
          <div className="space-y-10">
            {[
              { label: 'Kinetic Threshold (Steps)', current: steps + 5234, target: 10000, color: 'from-emerald-500 to-teal-500' },
              { label: 'Metabolic Threshold (Cal)', current: Math.floor(calories + 234), target: 500, color: 'from-orange-500 to-red-600' },
              { label: 'Temporal Threshold (Min)', current: Math.floor(activeMinutes + 28), target: 60, color: 'from-emerald-500 to-teal-600' },
            ].map((goal, i) => (
              <div key={i}>
                <div className="flex items-end justify-between mb-4">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-bold">{goal.label}</span>
                  <span className="text-xl font-black text-foreground">{goal.current.toLocaleString()} <span className="text-muted-foreground text-xs">/ {goal.target}</span></span>
                </div>
                <div className="h-4 bg-muted/60 rounded-full overflow-hidden border border-border/50 relative">
                  <motion.div
                    initial={{ width: 0 }}
        ...
        >
              ...
              <div key={index} className="flex-1 flex flex-col items-center group h-full justify-end">
                <div className="flex-1 w-full flex flex-col justify-end gap-1.5 relative mb-4">
                  ...
                  <motion.div
                    ...
                    className="w-full bg-gradient-to-t from-emerald-600 to-teal-600 rounded-t-xl group-hover:brightness-125 transition-all cursor-pointer relative shadow-lg"
                  >
                    <div className="absolute -top-10 left-1/2 -translate-x-1/2 glass px-3 py-1 rounded-lg text-[10px] font-black text-foreground opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border border-border/50 shadow-2xl pointer-events-none z-20">
          ...
          <div className="flex items-center justify-center gap-8 mt-10 pt-8 border-t border-border/50">
            <div className="flex items-center gap-3">
              <div className="w-4 h-4 rounded-md bg-gradient-to-r from-emerald-500 to-teal-500 shadow-lg" />
      ...
      >
        <div className="flex items-center gap-4 mb-10">
          <h2 className="text-2xl font-black uppercase tracking-tight text-foreground">Verification History</h2>
          ...
          {recentActivities.map((activity, i) => (
            ...
            >
              <div className="flex items-center gap-8">
                ...
                <div>
                  <p className="text-xl font-black uppercase tracking-tighter text-foreground transition-colors group-hover:text-primary">{activity.title}</p>
