...
func (s *UserService) GetDashboardStats(ctx context.Context, userID string) (*models.DashboardStats, error) {
	...
	countQuery := `
		...
		INNER JOIN public.competitions c ON cp.competition_id = c.id
...
func (s *UserService) GetUserProfile(ctx context.Context, userID string) (*models.UserProfile, error) {
	...
	existsQuery := `SELECT COUNT(*) FROM public.users WHERE id = $1`
	var count int
	...
	query := `
		SELECT
			u.id, u.email, COALESCE(u.name, 'User'),
		...
		LEFT JOIN public.leaderboard_entries le ON le.user_id = u.id AND le.competition_id = c.id
